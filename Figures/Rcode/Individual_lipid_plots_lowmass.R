...
for (i in 1:5){
  ...
  MS2s_rt <- getSpectrum(MS2_dat_lowmass[[i]], "rt", 
   ...
   MS2s_mz <- getSpectrum(MS2s_rt,  "precursor", 
                          ...
                          mz.tol = .1)
   if (length(MS2s_mz)==0){
     MS2s[[i]] = tibble(mz = NA, intensity = 0, has_As = FALSE, rt = NA) %>%
...
g2 <- ggplot(data = MS2s_topfrags, 
             aes(x = mz,xend = mz, 
                 y = 0, yend = intensity, 
                 color = has_As)) +
  geom_segment(data = MS2s_topfrags %>% 
                 filter(has_As == TRUE), color = 'coral1', 
               size = 1)+
  geom_segment()+
  geom_hline(yintercept = 0)+
  geom_text(data = MS2s_topfrags, 
...
g3 <- plot_grid(g, g2, align = 'h', ncol =2, rel_widths = c(1, 1.3))
g3
